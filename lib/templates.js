...
function generateSectPr(options = {}) {
  ...
  let marginTopInches = options.marginTop !== undefined ? options.marginTop : 1;
  let marginRightInches = options.marginRight !== undefined ? options.marginRight : 1;
  let marginBottomInches = options.marginBottom !== undefined ? options.marginBottom : 1;
  let marginLeftInches = options.marginLeft !== undefined ? options.marginLeft : 1;

  if (options.enablePageBorder || options.pageBorder) {
    marginTopInches -= 0.3 * marginTopInches;
    marginRightInches -= 0.3 * marginRightInches;
    marginBottomInches -= 0.3 * marginBottomInches;
    marginLeftInches -= 0.3 * marginLeftInches;
  }

  const marginTop = Math.round(marginTopInches * 1440);
  const marginRight = Math.round(marginRightInches * 1440);
  const marginBottom = Math.round(marginBottomInches * 1440);
  const marginLeft = Math.round(marginLeftInches * 1440);
  ...
  // Page borders
  if (options.enablePageBorder || options.pageBorder) {
    if (options.pageBorder && options.pageBorder.radius !== undefined) {
      // Use VML shape for page borders when radius is specified (even if 0)
    } else {
  ...
  if (options.enableHeader || (options.header && options.enableHeader !== false)) {
    const headerHeight = options.headerHeight !== undefined ? Math.round(options.headerHeight * 1440) : undefined;
    const heightAttr = headerHeight !== undefined ? ` w:h="${headerHeight}"` : '';
    sectPrContent += `
    <w:headerReference w:type="default" r:id="rId${nextId}"${heightAttr}/>`;
    nextId++;
  }

  if (options.enableFooter || (options.footer && options.enableFooter !== false)) {
    const footerHeight = options.footerHeight !== undefined ? Math.round(options.footerHeight * 1440) : undefined;
    const heightAttr = footerHeight !== undefined ? ` w:h="${footerHeight}"` : '';
    sectPrContent += `
    <w:footerReference w:type="default" r:id="rId${nextId}"${heightAttr}/>`;
  }
