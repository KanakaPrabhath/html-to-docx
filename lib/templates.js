...
function generateRoundedBorderShape(options = {}) {
  const borderConfig = options.pageBorder || {};
  const radius = borderConfig.radius || 0.25;
  const color = borderConfig.color || '000000';
  const size = borderConfig.size || 4;
  // Calculate page size
  const pageSize = options.pageSize || 'A4';
  let pageWidth, pageHeight;
  switch (pageSize.toUpperCase()) {
  ...
  const topPt = marginTopPt - marginHeaderPt;
  
  // Border settings
  const borderWeightPt = (size / 8) * 0.75; // Convert to pt (approximate)
  const strokeAttr = 'stroked="t"';
  const strokeElement = `<v:stroke color="#${color}" weight="${borderWeightPt}pt"/>`;
  const arcsize = ` arcsize="${radius}"`;
  ...
  <w:p>
    <w:r>
      <w:pict>
        <v:roundrect id="_x0000_s${shapeId}" style="position:absolute;left:0pt;top:${topPt}pt;width:${widthPt}pt;height:${heightPt}pt;" fillcolor="transparent" ${strokeAttr}${arcsize}>
          ${strokeElement}
        </v:roundrect>
      </w:pict>
    </w:r>
