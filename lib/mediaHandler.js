 ...
 */
export async function createFullWidthImageOOXML(base64Image, mediaManager, type = 'header') {
  try {
    ...
    // Page width - full width for edge-to-edge headers/footers (no margins subtracted)
    // US Letter: 8.5 inches = ~7770400 EMUs
    const pageWidthEmu = 7770400; // 8.5 inches in EMUs
    const imageWidthEmu = pageWidthEmu; // Full page width

    // For header/footer, height should be reasonable
    const imageHeightEmu = type === 'header' ? 2880000 : 2880000; // 2 inches for both header and footer
      ...
      <w:drawing>
        <wp:anchor distT="0" distB="0" distL="0" distR="0" simplePos="0" relativeHeight="0" behindDoc="0" locked="0" layoutInCell="0" allowOverlap="0">
          ...
          <wp:positionV relativeFrom="page">
            <wp:posOffset>${type === 'header' ? '0' : '8229600'}</wp:posOffset>
          </wp:positionV>
