...
export class MediaManager {
  ...
  getMediaFiles() {
    ...
    generateDocumentRelsXml(options = {}) {
      ...
      if (options.enableHeader || options.header) {
      ...
      if (options.enableFooter || options.footer) {
  ...
  generateContentTypesXml(options = {}) {
    ...
    if (options.enableHeader || (options.header && options.enableHeader !== false)) {
    ...
    if (options.enableFooter || (options.footer && options.enableFooter !== false)) {
...
function getPageDimensionsInEmu(pageSize = 'A4') {
  ...
  if (typeof pageSize === 'string') {
    const size = pageSizes[pageSize.toUpperCase()];
    if (size) {
  ...
  const widthEmu = Math.round(widthInches * EMU_PER_INCH);
  const heightEmu = Math.round(heightInches * EMU_PER_INCH);
 ...
 * @param {Object} options - Image options (width, height, float, margin, display, sectionHeader)
 * @returns {string} - OOXML image element
 */
export function createImageOOXML(mediaFile, imageOptions = {}) {
  const width = imageOptions.width || 300; // Default width in pixels
  const height = imageOptions.height || 200; // Default height in pixels

  // Convert pixels to EMUs (English Metric Units) - 1 inch = 914400 EMUs, 96 DPI
  const widthEmu = Math.round(width * EMU_PER_INCH / PIXELS_PER_INCH);
  const heightEmu = Math.round(height * EMU_PER_INCH / PIXELS_PER_INCH);

  // Use unique IDs for each image
  const imageId = mediaFile.imageId || 1;

  // Check if image should be positioned as section header
  if (imageOptions.sectionHeader) {
    return createSectionHeaderImageOOXML(mediaFile, imageOptions, widthEmu, heightEmu, imageId);
  }

  // Check if image should be floating
  const isFloating = imageOptions.float === 'left' || imageOptions.float === 'right';

  if (isFloating) {
    // Create anchored (floating) image
    return createAnchoredImageOOXML(mediaFile, imageOptions, widthEmu, heightEmu, imageId);
  } else {
    // Create inline image
 ...
 */
function createAnchoredImageOOXML(mediaFile, imageOptions, widthEmu, heightEmu, imageId) {
  const float = imageOptions.float;
  const margin = imageOptions.margin || {};

  // Convert margin pixels to EMUs
  const marginTop = margin.top ? Math.round(margin.top * EMU_PER_INCH / PIXELS_PER_INCH) : 0;
  const marginRight = margin.right ? Math.round(margin.right * EMU_PER_INCH / PIXELS_PER_INCH) : 0;
  const marginBottom = margin.bottom ? Math.round(margin.bottom * EMU_PER_INCH / PIXELS_PER_INCH) : 0;
  const marginLeft = margin.left ? Math.round(margin.left * EMU_PER_INCH / PIXELS_PER_INCH) : 0;
...
/**
 * Create section header image OOXML (positioned at top of page)
 * @param {Object} mediaFile - Media file object
 * @param {Object} options - Image options (including pageSize for cover images)
 * @param {number} widthEmu - Width in EMUs
 * @param {number} heightEmu - Height in EMUs
 * @param {number} imageId - Unique image ID
 * @returns {string} - OOXML section header image element
 */
function createSectionHeaderImageOOXML(mediaFile, options, widthEmu, heightEmu, imageId) {
  ...
  if (options.cover) {
    // For cover images, use full page width ignoring margins
    const pageSize = options.pageSize || 'A4';
    const { width: pageWidthEmu } = getPageDimensionsInEmu(pageSize);
    finalWidthEmu = pageWidthEmu;
  } else {
    // For non-cover section header images, use margins for positioning
    const margin = options.margin || {};
    marginTop = margin.top ? Math.round(margin.top * EMU_PER_INCH / PIXELS_PER_INCH) : 0;
    marginRight = margin.right ? Math.round(margin.right * EMU_PER_INCH / PIXELS_PER_INCH) : 0;
    marginBottom = margin.bottom ? Math.round(margin.bottom * EMU_PER_INCH / PIXELS_PER_INCH) : 0;
    marginLeft = margin.left ? Math.round(margin.left * EMU_PER_INCH / PIXELS_PER_INCH) : 0;

    // Position the image relative to page margins
    const pageMarginTop = options.marginTop ? Math.round(options.marginTop * EMU_PER_INCH) : Math.round(1 * EMU_PER_INCH);
    const pageMarginLeft = options.marginLeft ? Math.round(options.marginLeft * EMU_PER_INCH) : Math.round(1 * EMU_PER_INCH);

    posOffsetH = pageMarginLeft + marginLeft;
    posOffsetV = pageMarginTop + marginTop;
  }
    ...
    <w:drawing>
      <wp:anchor distT="0" distB="0" distL="0" distR="0" simplePos="0" relativeHeight="0" behindDoc="0" locked="0" layoutInCell="0" allowOverlap="0">
        ...
        <wp:positionH relativeFrom="page">
          <wp:posOffset>${posOffsetH}</wp:posOffset>
        </wp:positionH>
        <wp:positionV relativeFrom="page">
          <wp:posOffset>${posOffsetV}</wp:posOffset>
        </wp:positionV>
        <wp:extent cx="${finalWidthEmu}" cy="${heightEmu}"/>
        <wp:effectExtent l="0" t="0" r="0" b="0"/>
        <wp:wrapTopAndBottom/>
        <wp:docPr id="${imageId}" name="${mediaFile.alt || 'Section Header Image'}"/>
        <wp:cNvGraphicFramePr>
          <a:graphicFrameLocks noChangeAspect="1"/>
        </wp:cNvGraphicFramePr>
        <a:graphic>
          <a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture">
            <pic:pic>
              <pic:nvPicPr>
                <pic:cNvPr id="${imageId}" name="${mediaFile.alt || 'Section Header Image'}"/>
                <pic:cNvPicPr/>
              </pic:nvPicPr>
              <pic:blipFill>
                <a:blip r:embed="${mediaFile.id}"/>
              ...
              <pic:spPr>
                <a:xfrm>
                  <a:off x="0" y="0"/>
                  <a:ext cx="${finalWidthEmu}" cy="${heightEmu}"/>
                </a:xfrm>
                <a:prstGeom prst="rect">
                  <a:avLst/>
                </a:prstGeom>
...
export async function createFullWidthImageOOXML(base64Image, mediaManager, type = 'header', options = {}) {
  try {
    ...
    const imageHeightEmu = Math.round(heightInches * EMU_PER_INCH);
    ...
    <w:r>
      <w:drawing>
        <wp:anchor distT="0" distB="0" distL="0" distR="0" simplePos="0" relativeHeight="0" behindDoc="${type === 'footer' ? '1' : '0'}" locked="0" layoutInCell="0" allowOverlap="${type === 'footer' ? '1' : '0'}">
          <wp:simplePos x="0" y="0"/>
          <wp:positionH relativeFrom="page">
