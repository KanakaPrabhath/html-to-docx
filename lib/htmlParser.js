...
import { createParagraph, createRun, createTextBox } from './ooxmlGenerator.js';
 ...
 */
export async function parseHtml(html, mediaManager = null, part = 'document') {
  ...
  for (const node of body.childNodes) {
    bodyContent += await processNode(node, mediaManager, part);
  }
 ...
 */
export async function processNode(node, mediaManager = null, part = 'document') {
  ...
  if (node.nodeType === 1) { // Element node
    ...
    switch (tagName) {
      case 'p':
        return await processParagraph(node, mediaManager, part);
      case 'h1':
      case 'h2':
      case 'h3':
      case 'h4':
      case 'h5':
      case 'h6':
        return await processHeading(node, mediaManager, part);
      case 'strong':
      case 'b':
        return await processInlineStyle(node, { bold: true }, mediaManager, part);
      case 'em':
      case 'i':
        return await processInlineStyle(node, { italic: true }, mediaManager, part);
      case 'u':
        return await processInlineStyle(node, { underline: true }, mediaManager, part);
      case 'strike':
      case 's':
        return await processInlineStyle(node, { strikethrough: true }, mediaManager, part);
      case 'br':
        return await createParagraph('', {});
      case 'span':
        return await processSpan(node, mediaManager, part);
      case 'div':
        return await processDiv(node, mediaManager, part);
      case 'ul':
      case 'ol':
        return await processList(node, mediaManager, part);
      case 'li':
        return await processListItem(node, listType, mediaManager, part);
      case 'img':
        const imageOOXML = await processImage(node, mediaManager, part);
        return imageOOXML ? `<w:p>${imageOOXML}</w:p>` : '';
      case 'table':
        return await processTable(node, mediaManager, part);
      case 'tr':
        return await processTableRow(node, mediaManager, part);
      case 'td':
      case 'th':
        return await processTableCell(node, mediaManager, part);
      case 'page-break':
      ...
      default:
        ...
        for (const child of node.childNodes) {
          content += await processNode(child, mediaManager, part);
        }
        return content;
    }
  }
 ...
 */
export async function processParagraph(element, mediaManager = null, part = 'document') {
  const styles = parseStyles(element);
  const text = getTextContent(element);

  const paragraphContent = await createParagraph(text, styles, element, async (el, parentStyles) => await processChildNodes(el, parentStyles, mediaManager, part));
 ...
 */
export async function processHeading(element, mediaManager = null, part = 'document') {
  ...
  const shouldWrapInTextBox = styles.backgroundColor && (styles.padding || styles.width || styles.borderRadius);

  if (shouldWrapInTextBox) {
    ...
    const headingContent = await createParagraph(text, styles, element, async (el, parentStyles) => await processChildNodes(el, parentStyles, mediaManager, part));
 ...
 * @returns {Promise<string>} - OOXML content
 */
export async function processInlineStyle(element, styleOverrides, mediaManager = null, part = 'document') {
  const styles = { ...parseStyles(element), ...styleOverrides };
  const text = getTextContent(element);
  return await createParagraph(text, styles, element, async (el, parentStyles) => await processChildNodes(el, parentStyles, mediaManager, part));
}
 ...
 * @returns {Promise<string>} - OOXML content
 */
export async function processSpan(element, mediaManager = null, part = 'document') {
  const styles = parseStyles(element);
  const text = getTextContent(element);
 ...
 * @returns {Promise<string>} - OOXML content
 */
export async function processDiv(element, mediaManager = null, part = 'document') {
  const styles = parseStyles(element);
  ...
  const shouldWrapInTextBox = styles.backgroundColor && (styles.padding || styles.border || styles.width || styles.borderRadius);
 ...
 * @returns {Promise<string>} - OOXML content
 */
export async function processList(element, mediaManager = null, part = 'document') {
  ...
  for (const child of element.children) {
    if (child.tagName.toLowerCase() === 'li') {
      content += await processListItem(child, listType, mediaManager, part);
    }
  }
 ...
 */
export async function processListItem(element, listType, mediaManager = null, part = 'document') {
  const styles = parseStyles(element);
  styles.listType = listType;
  styles.listItem = true;
  const text = getTextContent(element);

  const listItemContent = await createParagraph(text, styles, element, async (el, parentStyles) => await processChildNodes(el, parentStyles, mediaManager, part));
 ...
 */
export async function processChildNodes(element, parentStyles, mediaManager = null, part = 'document') {
  let runs = '';
    ...
    if (node.nodeType === 3) {
      ...
      switch (tagName) {
        ...
        case 'b':
          ...
          runs += await processChildNodes(node, childStyles, mediaManager, part);
          break;
        case 'em':
        case 'i':
 ...
 */
export async function processTable(element, mediaManager = null, part = 'document') {
  const styles = parseStyles(element);
  let tableContent = '';
  ...
  for (const row of rows) {
    tableContent += await processTableRow(row, mediaManager, part);
  }
 ...
 */
export async function processTableRow(element, mediaManager = null, part = 'document') {
  const styles = parseStyles(element);
  let rowContent = '';
  ...
  for (const cell of cells) {
    rowContent += await processTableCell(cell, mediaManager, part);
  }
 ...
 */
export async function processTableCell(element, mediaManager = null, part = 'document') {
  const styles = parseStyles(element);
  const isHeader = element.tagName.toLowerCase() === 'th';
  const text = getTextContent(element);
  
  const cellContent = await createParagraph(text, styles, element, async (el, parentStyles) => await processChildNodes(el, parentStyles, mediaManager, part));
