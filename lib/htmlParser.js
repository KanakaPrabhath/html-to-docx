...
export function processNode(node) {
  ...
  if (node.nodeType === 1) { // Element node
    ...
    switch (tagName) {
      ...
      case 'strike':
      case 's':
        return processInlineStyle(node, { strikethrough: true });
      case 'br':
...
export function processList(element) {
  ...
  for (const child of element.children) {
    if (child.tagName.toLowerCase() === 'li') {
      content += processListItem(child, listType);
    }
  }
 ...
 */
export function processListItem(element, listType) {
  const styles = parseStyles(element);
