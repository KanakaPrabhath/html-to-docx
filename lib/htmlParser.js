...
import { createParagraph, createRun, createParagraphProperties } from './ooxmlGenerator.js';
...
function shouldWrapInTextBox(styles) {
  return (styles.backgroundColor || styles.gradient) && (styles.padding || styles.width || styles.borderRadius);
}
...
export async function processInlineStyle(element, styleOverrides, mediaManager = null, options = {}, part = 'document') {
  ...
  // For headers and footers, return runs instead of a full paragraph
  if (part === 'header' || part === 'footer') {
    return await processChildNodes(element, styles, mediaManager, options, part);
  }
...
export function processSpan(element, mediaManager = null, part = 'document') {
  ...
  // For headers and footers, return runs instead of a paragraph
  if (part === 'header' || part === 'footer') {
    return createRun(text, styles);
  }
  
  return createParagraph(text, styles);
}
...
export async function processDiv(element, mediaManager = null, options = {}, part = 'document') {
  ...
  // For headers and footers, collect all content as runs in a single paragraph
  if (part === 'header' || part === 'footer') {
    const runs = await processChildNodes(element, styles, mediaManager, options, part);
    if (runs.trim()) {
      return `<w:p>${createParagraphProperties(styles)}${runs}</w:p>`;
    }
    return '';
  }
 ...
 */
export async function processImage(element, mediaManager = null, options = {}, part = 'document') {
  if (!mediaManager) {
  ...
  const isSectionHeader = element.hasAttribute('data-section-header');
  const isCover = element.hasAttribute('data-cover');
  ...
  try {
    ...
    if (mediaFile) {
      ...
      return createImageOOXML(mediaFile, imageOptions);
    }
  } catch (error) {
...
export async function processTable(element, mediaManager = null, options = {}, part = 'document') {
  ...
  return createTable(tableContent, styles, element);
}
