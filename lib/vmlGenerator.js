...
export function generateRoundedBorderShape(options = {}) {
  ...
  let strokeElement = `<v:stroke color="#${color}" weight="${borderWeightPt}pt"`;
  ...
  if (style === 'double') {
    ...
    strokeElement = `<v:stroke color="#${color}" weight="${Math.max(borderWeightPt * 1.5, borderWeightPt + 2)}pt"`;
  }
  ...
  const normalizedRadius = Math.min(Math.max(radius / Math.min(widthPt, heightPt), 0), 1);
