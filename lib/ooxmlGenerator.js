...
function parsePadding(padding) {
  ...
  if (parts.length === 1) {
    const val = parseInt(parts[0]) * 20; // Convert to twentieths of a point
    paddings.top = paddings.right = paddings.bottom = paddings.left = val;
  } else if (parts.length === 2) {
    paddings.top = paddings.bottom = parseInt(parts[0]) * 20;
    paddings.right = paddings.left = parseInt(parts[1]) * 20;
  } else if (parts.length === 3) {
    paddings.top = parseInt(parts[0]) * 20;
    paddings.right = paddings.left = parseInt(parts[1]) * 20;
    paddings.bottom = parseInt(parts[2]) * 20;
  } else if (parts.length === 4) {
    paddings.top = parseInt(parts[0]) * 20;
    paddings.right = parseInt(parts[1]) * 20;
    paddings.bottom = parseInt(parts[2]) * 20;
    paddings.left = parseInt(parts[3]) * 20;
  }
