...
export async function convertHtmlToDocx(html, options = {}) {
  ...
  if (options.header && options.enableHeader !== false) {
    if (options.header.startsWith('data:image/')) {
      ...
      // Handle HTML header
      headerContent = await parseHtml(options.header, mediaManager, 'header');
    }
  }
  ...
  if (options.footer && options.enableFooter !== false) {
    if (options.footer.startsWith('data:image/')) {
      ...
      // Handle HTML footer
      footerContent = await parseHtml(options.footer, mediaManager, 'footer');
    }
  }

  // Add page numbers to footer if enabled
  if (options.enablePageNumbers) {
    const pageNumberAlignment = options.pageNumberAlignment || 'right';
    const pageNumberOOXML = generatePageNumberOOXML(pageNumberAlignment);
    if (footerContent) {
      // Append page number to existing footer
      footerContent += pageNumberOOXML;
    } else {
      // Create new footer with page number
      footerContent = pageNumberOOXML;
    }
  }
