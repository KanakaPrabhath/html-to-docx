...
export async function convertHtmlToDocx(html, options = {}) {
  ...
  if (options.header) {
    if (options.header.startsWith('data:image/')) {
      ...
      headerContent = await createFullWidthImageOOXML(options.header, mediaManager, 'header');
    } else {
      // Handle HTML header
  ...
  if (options.footer) {
    if (options.footer.startsWith('data:image/')) {
      ...
      footerContent = await createFullWidthImageOOXML(options.footer, mediaManager, 'footer');
    } else {
      // Handle HTML footer
