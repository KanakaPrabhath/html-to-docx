...
export function cleanHtml(html) {
  ...
  cleaned = cleaned.replace(/<[^>]+style="[^"]*"[^>]*>/gi, (match) => {
    ...
    if (tagName === 'img') {
      ...
      allowedProperties = ['text-align', 'font-weight', 'font-style', 'text-decoration', 'color', 'background-color', 'background', 'font-size', 'font-family', 'border-collapse', 'width', 'height', 'border', 'padding', 'border-radius', 'vertical-align'];
    }
...
export function parsePadding(padding) {
  ...
  // Helper function to convert padding value to DXA (twentieths of a point)
  const convertToDxa = (value) => {
    const numValue = parseFloat(value);
  ...
  if (parts.length === 1) {
    const val = convertToDxa(parts[0]);
    paddings.top = paddings.right = paddings.bottom = paddings.left = val;
  } else if (parts.length === 2) {
    paddings.top = paddings.bottom = convertToDxa(parts[0]);
    paddings.right = paddings.left = convertToDxa(parts[1]);
  } else if (parts.length === 3) {
    paddings.top = convertToDxa(parts[0]);
    paddings.right = paddings.left = convertToDxa(parts[1]);
    paddings.bottom = convertToDxa(parts[2]);
  } else if (parts.length === 4) {
    paddings.top = convertToDxa(parts[0]);
    paddings.right = convertToDxa(parts[1]);
    paddings.bottom = convertToDxa(parts[2]);
    paddings.left = convertToDxa(parts[3]);
  }
